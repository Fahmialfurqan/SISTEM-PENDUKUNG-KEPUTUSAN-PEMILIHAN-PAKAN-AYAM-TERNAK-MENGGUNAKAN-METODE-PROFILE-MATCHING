...
if (isset($_POST['proses'])) {
    ...
    while ($r_alt = mysqli_fetch_array($q_alt)) {
        ...
        if (mysqli_num_rows(mysqli_query($con, "select * from hasil where id_alternatif='" . $id_alternatif . "'")) > 0) {
            $q = "update hasil set nilai='" . $nilai_pm . "' where id_alternatif='" . $id_alternatif . "'";
            mysqli_query($con, $q);
        } else {
            $q = "insert into hasil(id_alternatif,nilai) values ('" . $id_alternatif . "','" . $nilai . "')";
            mysqli_query($con, $q);
        }
    }
    ...
    foreach ($hasil as $arr) {
        ...
        $ket = mysqli_fetch_array(mysqli_query($con, "select Keterangan from hasil where id_alternatif='$id_alternatif'"));
